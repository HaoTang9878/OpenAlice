 ...
 *
 * Separate from OpenAlpha's `EngineContext` (`@/core/types.js`) by design —
 * OpenAlpha holds the HTTP SDK adapter `UTAManagerSDK` (`resolve()` returns
 ...
 * anything else off ctx, add it here — not via casting back to OpenAlpha's
 * `EngineContext`.
 */
