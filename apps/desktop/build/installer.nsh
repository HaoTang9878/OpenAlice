!macro customInit
  ${if} ${isUpdated}
    DetailPrint "Closing the legacy OpenAlpha process tree before update."
    nsExec::ExecToLog '"$SYSDIR\taskkill.exe" /T /F /IM "${APP_EXECUTABLE_FILENAME}"'
    Pop $0
    DetailPrint "Closing remaining processes launched from the OpenAlpha install directory."
    ...
    ; removes the app directory without touching Electron/OpenAlpha user data,
    ...
    DetailPrint "Removing the legacy OpenAlpha app directory with long-path support."
    ...
    Pop $0
    ${if} $0 != 0
      DetailPrint "Unable to remove the legacy OpenAlpha app directory (exit $0)."
      SetErrorLevel 2
      Quit
    ${endif}
