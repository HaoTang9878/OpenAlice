...
it('round-trips a Codex free-text question in demo mode', async () => {
  ...
  const result = await postJson(`${url}/respond`, { requestId: request.id, optionId: '', text: 'OpenAlpha research' })
  expect(result.status).toBe(200)
  expect(result.body.snapshot.requests).toEqual([])
  expect(JSON.stringify(result.body.snapshot.messages)).toContain('Project name: OpenAlpha research')
})
